...
def createData(num_iteration):
    ...
    for i in range(0, pre_change_days):
        ...
        Y_l0, Y_l1 = gaussian_data(n, mean11, mean21_86, covariance11, covariance21, new_seed)
        #print ("i:",i)
        data = np.concatenate((Y_l0,Y_l1))
        GaussianSamples86 = np.append(GaussianSamples86, np.array(data), axis=0) 
    
    #Save the data
    print ("Saving data for 1000 pre-change days, 10Simulations, iteration#:",num_iteration)
    np.save(('/home/smriti.prathapan/note1/GaussData-March0724/Mar18-2024/D86/%d-Gaussian86-10S-1000D.npy' %num_iteration), GaussianSamples86) # save
