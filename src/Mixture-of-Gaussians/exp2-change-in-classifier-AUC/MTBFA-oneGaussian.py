...
def cusum_detection(limit,ref_val):
    # Define the mean for Gaussian in Class1
    mean11 = np.matrix([[0.], [0.]])
    ...
    #--------------------------------------------------------------------------------
    print("Reading Data")
    #os.chdir("/home/smriti.prathapan/note1/GaussData-March0724/") # Data 1 
    #GaussianSamples86 = np.load('G86-March0724-3000days1000Sim.npy')     #in-control 
    #os.chdir("/home/smriti.prathapan/note1/GaussData-March0724/") 
    #GaussianSamples83 = np.load('G83-March0724-3000days1000Sim.npy')     #out-of-control 2-Gaussian83-10S-1000D.npy
    os.chdir("/home/smriti.prathapan/note1/GaussData-March0724/Data-seq-March10")
    GaussianSamples86 = np.load('G86-March1024-3000days1000Sim.npy')     #in-control 
    GaussianSamples83 = np.load('G83-March1024-3000days1000Sim.npy')     #out-of-control
    ...
    start_83         = 0
    end_83           = 1
    #delta            = 1 #0.616  #0.1481 #0.318 try 1, 2 and 3
    #delta            = ref_val
    control_limit    = limit
    ...
    while (runs < 1000):   #was 8 from the 10S 1000 D data
        ...
        #delta  = 2
        h      = in_std_auc * control_limit 
        #k      = (delta * in_std_auc)/2  ref_val
        ...
        AUC_in           = np.append(AUC_in, mu)    # this is the in-control AUC for one experiment
        AUC_out          = np.append(AUC_out, mu_1) # this is the out-of-control AUC for one experiment 
    ...
    print ("mu_0", mu)
    print ("mu_1", mu_1)
    print("Mean in-control AUC:\t", np.mean(sp_pre)) #change the variable name to AUC
    print("Mean out-of-control AUC:\t", np.mean(sp_post)) #change the variable name to AUC
    nonZeroAvgDD = AvgDD[np.nonzero(AvgDD)]

    #Convert the AUCs ans in_std to dataframe
    #out_AUC_in = pd.DataFrame(AUC_in) 
    #out_AUC_out = pd.DataFrame(AUC_out)
    #out_inSTD_test_AUCs = pd.DataFrame(inSTD_test_AUCs)
    #out_Displacement = pd.DataFrame(Displacement)


    #Save the Convert the AUCs and in_std to a csv file
    #out_AUC_in.to_csv("/home/smriti.prathapan/note1/AUC_in.csv")
    #out_AUC_out.to_csv("/home/smriti.prathapan/note1/AUC_out.csv")
    #out_inSTD_test_AUCs.to_csv("/home/smriti.prathapan/note1/inSTD_test_AUCs.csv")
    #out_Displacement.to_csv("/home/smriti.prathapan/note1/Displacement.csv")

    # Construct the tabular CUSUM Chart
    #AUC_chart = np.array([])
    #AUC_chart = np.column_stack((AUC_in.T, inSTD_test_AUCs.T, AUC_out.T, Displacement.T))
    #df_AUC_chart = pd.DataFrame(AUC_chart) 
    #df_AUC_chart.columns = ['AUC_in','sigma_in','AUC_out', 'Displacement']

    #df_AUC_chart.to_csv("/home/smriti.prathapan/note1/AUC_chart.csv")
    ...
    print("Writing the pre- and post-change AUCs\n")
    # save the 1000 simulations * 1000 pre- and pos-change AUCs as a csv file 
    preChange_AUC_1000S_1000D.to_csv("/home/smriti.prathapan/note1/OUT-Data2/preChange_AUC_1000S_1000Dh5k%d.csv" %ref_val)
    postChange_AUC_1000S_1000D.to_csv("/home/smriti.prathapan/note1/OUT-Data2/postChange_AUC_1000S_1000Dh5k%d.csv" %ref_val)
    ...
    df_delay_chart.to_csv("/home/smriti.prathapan/note1/OUT-Data2/delay_charth5k%d.csv" %ref_val)

if __name__ == "__main__":
    print("Number of arguments:", len(sys.argv))
    print("Argument List:", str(sys.argv))

    control_limit, ref_val = float(sys.argv[1]), float(sys.argv[2])
    cusum_detection (control_limit, ref_val)
